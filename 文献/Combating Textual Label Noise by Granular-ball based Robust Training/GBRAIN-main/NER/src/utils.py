                    ...
                    for index,lb in enumerate(labels):
                        if 'O'!=lb: 
                            if 'LOC' in lb:
                                ...
                                kk=1 
                            start = max(0,index-5)
                            end = min(len(labels),index+5)
                            
                            new_text = ' '.join(text.split(' ')[start:end])
                            new_label = labels[start:end]
                            
                            pad_data.extend([(new_text,new_label)]*kk)
            ...
            for text, labels in tqdm(all_data, desc="Converting to tensors"):
                ...
                for i, token in enumerate(tokens[1:], start=1):  # skip [CLS]
                    ...
                    if i == 1 or  token.startswith('Ġ'):
                        ...
                        valid_pos[i] = 1
                        j += 1
                    elif ( not token.startswith('Ġ')) and dataset_name == 'train':
                        
                        label_idx[i] = label_idx[i-1]
                        valid_pos[i]=1
