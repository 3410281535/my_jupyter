    ...
    for file_name in [train_file, test_file]:
        ...
        for label, text in csv_file:
            ...
            if preprocessing:
                ...
                elif len(text.split())<text_length:
                    text = text.split() +['<pad>']*(text_length-len(text.split()))
                    text = " ".join(text)
...
def trec_dataset(directory='data/',preprocessing=False,fine_grained=False,verbose=False,text_length=5000):
    share_id = '1BSqCU6DwIVD1jllbsz9ueudu3tSfomzY' if preprocessing else '11-l1T-_kBdtqrqqSfqNnJCLC6_NRM9Je'
    return imdb_dataset(directory, 'trec', preprocessing, fine_grained, verbose, text_length, share_id)